...
export default function OcrPrepPanel({ data, currentPage, onRunOcr, onToggleOcrTarget }: Props) {
  ...
  const isTextLikePage = page.pageType === "text" || page.pageType === "table_candidate";
  const isImagePage = page.pageType === "image";
  const guidance = isTextLikePage
    ? {
        title: "PDF文字を取得できています。",
        description: "通常はOCRを使わなくても読み取れます。必要な場合はOCRで再確認できます。",
        buttonLabel: "このページをOCRで再解析する",
      }
    : isImagePage
      ? {
          title: "画像中心のページです。",
          description: "OCR解析をおすすめします。",
          buttonLabel: "このページをOCR解析する",
        }
      : {
          title: "一部の文字を取得できない可能性があります。",
          description: "必要に応じてOCR解析できます。",
          buttonLabel: "このページをOCR解析する",
        };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      <div style={{ backgroundColor: "#fef2f2", padding: "1rem", borderRadius: "8px", border: "1px solid #fca5a5" }}>
        ...
        <div style={{ backgroundColor: "white", padding: "1rem", borderRadius: "4px", marginBottom: "1rem" }}>
          <p style={{ margin: "0 0 0.5rem 0", fontWeight: 600 }}>{guidance.title}</p>
          <p style={{ margin: 0, color: "#374151" }}>{guidance.description}</p>
        </div>
        
        <div style={{ backgroundColor: "white", padding: "1rem", borderRadius: "4px", marginBottom: "1rem" }}>
          <ul style={{ margin: 0, paddingLeft: "1.2rem", color: "#374151" }}>
            <li><strong>対象ページ番号:</strong> {page.pageNumber}</li>
            <li><strong>現在の判定:</strong> {page.pageType}</li>
        ...
        <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
          ...
          >
            {isProcessing ? "OCR解析中..." : guidance.buttonLabel}
          </button>
